...
class FakeTaperRepository(
    ...
    override fun observeHabit(id: Long): Flow<Habit?> =
        habitsFlow.map { habits -> habits.firstOrNull { it.id == id } }
    ...
    override suspend fun updateHabitAndPlan(habit: Habit) {
        habitsFlow.value = habitsFlow.value.map { existing ->
    ...
    fun setEvents(habitId: Long, events: List<HabitEvent>) {
        eventsFlow.value = eventsFlow.value.toMutableMap().apply {
            put(habitId, events)
        }
    }
}
